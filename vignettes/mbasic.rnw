...
When using \Rfunction{MBASIC} to analyze data with the negative binomial distribution, the 'min.count' can be used to specify the minimum data threshold for enriched components. For ChIP-seq data, this can be used as a threshold for the enrichment detection. For the previous example, if we want to add a restriction that all loci with states other than 1 must have values at least 5, we can use:
...
We can also use 'min.count' to specify different minimum data thresholds for each component:

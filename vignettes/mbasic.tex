...
\hlstd{pred.states} \hlkwb{<-} \hlkwd{as.character}\hlstd{(}\hlkwd{apply}\hlstd{(fit}\hlopt{@}\hlkwc{Theta}\hlstd{[}\hlkwd{rownames}\hlstd{(fit}\hlopt{@}\hlkwc{Theta}\hlstd{)} \hlopt{==} \hlstd{conds[repid], ],} \hlnum{2}\hlstd{, which.max))}
\hlkwd{ggplot}\hlstd{()} \hlopt{+} \hlkwd{geom_point}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= input.counts,} \hlkwc{y} \hlstd{= chip.counts,} \hlkwc{color} \hlstd{= pred.states))} \hlopt{+} \hlkwd{theme}\hlstd{(}\hlkwc{legend.position} \hlstd{=} \hlstr{"top"}\hlstd{)}
\end{alltt}
\end{kframe}
